...
export default function AdminUsers() {
  ...
  const checkSupabaseConfig = async () => {
    try {
      ...
      if (!data.configured) {
        setError('External authentication is not configured. Please check your environment variables.')
        setLoading(false)
        return
      }
          ...
          <div className={styles.alertBanner}>
            <div className={styles.alertContent}>
              ...
              <div className={styles.alertText}>
                <strong>Action Required:</strong> {pendingUsers.length} user{pendingUsers.length > 1 ? 's' : ''} waiting for approval
              </div>
              <button 
                ...
                {users.map((userItem) => (
                  <div key={userItem.id} className={styles.tableRow}>
                    ...
                    <div className={styles.role}>
                      <select
                        value={userItem.role}
                        onChange={(e) => handleUpdateRole(userItem.id, e.target.value as 'admin' | 'user')}
                    ...
                    <div className={`${styles.status} ${userItem.approved ? styles.approved : styles.pending}`}>
                      <div className={styles.selectWrapper}>
                        <select
                          value={userItem.approved ? 'approved' : 'pending'}
                          onChange={(e) => handleUpdateStatus(userItem.id, e.target.value === 'approved')}
