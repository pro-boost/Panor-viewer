...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!isSupabaseConfigured()) {
    return res.status(500).json({ 
      error: 'Authentication system is not configured. Please contact the administrator.' 
    });
  }
  ...
  if (strength < 3) {
    return res.status(400).json({ 
      error: 'Password is too weak. Please include uppercase, lowercase, and numbers.' 
    });
  }

  try {
    // Attempt to register the user with Supabase
    const result = await authHelpers.signUp(email.toLowerCase().trim(), password);
    ...
    if (signUpError.message?.includes('email confirmation')) {
      return res.status(400).json({ 
        error: 'Email confirmation is required. Please check your email and follow the confirmation link.' 
      });
    }
    ...
    return res.status(500).json({ 
      error: 'Registration failed. Please try again later or contact support if the problem persists.' 
    });
  }
}