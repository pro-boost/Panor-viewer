...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (method === 'POST') {
    ...
    try {
      ...
      if (hasAdminUsers) {
        return res.status(409).json({ error: 'Authentication is already configured' });
      }
      ...
      try {
        ...
        console.log('Existing profile check:', { userId: user.id, existingProfile });
        ...
        if (existingProfile) {
          ...
          if (!success) {
            return res.status(500).json({ error: 'Failed to update user to admin role' });
          }
        } else {
          ...
          if (!success) {
            console.error('Failed to create admin profile for user:', { userId: user.id, email });
        ...
        if (signUpError.code === 'user_already_exists' || signUpError.message?.includes('User already registered')) {
          ...
          try {
            ...
            if (!user) {
              return res.status(409).json({ 
                error: 'User already exists but cannot sign in. Please check your credentials or contact support.' 
              });
            }
            ...
            if (existingProfile) {
              if (existingProfile.role === 'admin') {
                return res.status(409).json({ 
                  error: 'Authentication is already configured with this email.' 
                });
              } else {
              ...
              if (!success) {
                console.error('Failed to create admin profile for existing user:', { userId: user.id, email });
                return res.status(500).json({ error: 'Failed to create admin profile for existing user' });
              }
            }
            
            return res.status(201).json({ 
              success: true, 
              message: 'Authentication configured successfully for existing user',
              user: {
                email: user.email,
                id: user.id,
                role: 'admin'
            ...
            if (profileError.message?.includes('Invalid login credentials')) {
              return res.status(409).json({ 
                error: 'User already exists but the password is incorrect. Please use the correct password or contact support.' 
              });
            }
            return res.status(500).json({ 
              error: 'User exists but failed to configure admin profile. Please contact support.' 
            });
          }
        }
        ...
        if (signUpError.message?.includes('email confirmation')) {
          return res.status(400).json({ 
            error: 'Email confirmation is required. Please disable email confirmation in your Supabase project settings or configure SMTP.' 
          });
        }
        ...
        if (signUpError.message?.includes('Database error saving new user')) {
          return res.status(500).json({ 
            error: 'Database setup error. Please ensure you have run the SQL schema from SUPABASE_AUTH.md in your Supabase SQL editor. The user_profiles table and trigger function may be missing.' 
          });
        }
