...
function loadIndividualPOI(
  individualDir: string,
  poiId: string,
): POIData | null {
...
function migrateToIndividualFiles(projectId: string): boolean {
  ...
  try {
    ...
    for (const poi of pois) {
      ...
      fs.writeFileSync(
        poiFilePath,
        JSON.stringify(poiPackage, null, 2),
        "utf8",
      );
    ...
    console.log(
      `Migrated ${pois.length} POIs to individual files for project ${projectId}`,
    );
    return true;
  } catch (error) {
...
export default async function handler(
  ...
  try {
    ...
    const shouldUseIndividual =
      useIndividual === "true" || fs.existsSync(indexFile);
    ...
    if (shouldUseIndividual) {
      ...
      if (poiId && typeof poiId === "string") {
        ...
        return res
          .status(200)
          .json({ pois, source: "individual", count: pois.length });
      }
    } else {
