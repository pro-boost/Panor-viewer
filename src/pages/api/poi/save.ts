...
export default async function handler(
  ...
  try {
    const {
      ...
      useIndividual,
      ...newPOI
    }: POIData & { projectId: string; useIndividual?: boolean } = req.body;

    // Validate required fields
    console.log("POI Save API - Received data:", {
      projectId,
      newPOI,
      useIndividual,
    });
    ...
    if (useIndividual) {
      const saveIndividualResponse = await fetch(
        `${req.headers.origin || "http://localhost:3000"}/api/poi/save-individual`,
        {
          method: "POST",
          headers: {
