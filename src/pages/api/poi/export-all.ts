...
export default async function handler(
  ...
  try {
    ...
    if (!fs.existsSync(poiDataPath)) {
      return res
        .status(404)
        .json({ error: "No POI data found for this project" });
    }
    ...
    for (const poi of pois) {
      if (
        poi.type === "file" &&
        poi.content &&
        !addedAttachments.has(poi.content)
      ) {
        ...
        if (fs.existsSync(attachmentPath)) {
          try {
            ...
            archive.append(attachmentBuffer, {
              name: `attachments/${poi.content}`,
            });
            addedAttachments.add(poi.content);
          } catch (fileError) {
            console.warn(
              `Failed to read attachment file: ${poi.content}`,
              fileError,
            );
