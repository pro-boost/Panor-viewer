          ...
          fs.copyFile(oldPath, newPath, (copyErr) => {
            if (copyErr) {
              ...
              fs.unlink(oldPath, (unlinkErr) => {
                if (unlinkErr) {
                  console.warn(
                    `Failed to delete temp file after copy: ${unlinkErr.message}`,
                  );
                }
                resolve();
              });
...
export default async function handler(
  ...
  try {
    ...
    const projectsPath =
      process.env.PROJECTS_PATH || path.join(process.cwd(), "projects");
    ...
    const imageFiles = Array.isArray(files.images)
      ...
      : files.images
        ? [files.images]
        : [];
    const existingImages = fields.existing_images
      ? Array.isArray(fields.existing_images)
    ...
    console.log(
      `Successfully moved ${movedImages.length} image files:`,
      movedImages,
    );

    // Handle POI file
    console.log(
      "Backend - checking for POI file in files object:",
      Object.keys(files),
    );
    const poiFile = Array.isArray(files.poiFile)
      ? files.poiFile[0]
      : files.poiFile;
    console.log(
      "Backend - POI file found:",
      !!poiFile,
      poiFile ? poiFile.originalFilename : "none",
    );
    if (poiFile) {
      ...
      if (originalFilename.toLowerCase().endsWith(".zip")) {
        ...
        try {
          ...
          const poiDataEntry = zipEntries.find(
            (entry: any) =>
        ...
        console.log(
          `Backend - Moving POI file from ${poiFile.filepath} to ${poiDestPath}`,
        );
        await moveFile(poiFile.filepath, poiDestPath);
    ...
    try {
      console.log(
        `Starting configuration generation for project: ${projectId}`,
      );
      ...
      if ((process as any).resourcesPath) {
        ...
        if (!fs.existsSync(scriptPath)) {
          console.log(
            `Script not found at calculated path ${scriptPath}, trying alternative paths...`,
          );

          // Try relative to the current working directory
          const altScriptPath1 = path.join(
            process.cwd(),
            "scripts",
            "node",
            "generate-config.js",
          ...
          if (fs.existsSync(altScriptPath1)) {
            ...
            const altScriptPath2 = path.join(
      ...
      console.log(
        `Script environment PROJECTS_PATH: ${scriptEnv.PROJECTS_PATH}`,
      );
      ...
      console.log(
        `Explicitly passing PROJECTS_PATH as argument: ${projectsPath}`,
      );
      ...
      if (scriptError.message) {
        if (scriptError.message.includes("CSV file not found")) {
          ...
          errorDetails =
            "Python or required packages (numpy) are not installed";
    ...
    if (error.code === 1009 || error.httpCode === 413) {
      errorMessage =
        "File upload failed due to size restrictions. Please try with smaller files.";
      statusCode = 413;
      ...
      errorMessage =
        "Too many files being processed. Please try again in a moment.";
      statusCode = 503;
