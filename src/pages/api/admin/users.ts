  ...
  try {
    ...
    if (user.id === userId) {
      ...
      if (action === 'updateStatus' && approved === false) {
        return res.status(400).json({ error: 'Cannot change your own approval status' });
      }
    }
    ...
    if (action === 'updateRole' && role !== 'admin') {
      ...
      if (targetUser?.role === 'admin') {
        ...
        const adminCount = allUsers.filter(u => u.role === 'admin' && u.approved).length;
        if (adminCount <= 1) {
    ...
    switch (action) {
      ...
      case 'updateRole':
        if (!role) {
          return res.status(400).json({ error: 'Role is required for updateRole action' });
        }
        success = await userService.updateUserRole(userId, role);
        break;
      case 'updateStatus':
        if (typeof approved !== 'boolean') {
          return res.status(400).json({ error: 'Approved status is required for updateStatus action' });
        }
