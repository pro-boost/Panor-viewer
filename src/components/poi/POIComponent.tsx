...
const POIComponent = React.forwardRef<POIComponentRef, POIComponentProps>(
  (
    ...
    const createDebugMarker = useCallback(
    ...
    const handleCreatePOI = () => {
      ...
      if (!currentPendingPOI) {
        toast.error(
          "No POI position available. Please try right-clicking again.",
        );
        return;
      }
      setShowContextMenu(false);
    ...
    const savePOI = async (data: POIFormData) => {
      ...
      const currentPendingPOI =
        data.position || pendingPOI || pendingPOIRef.current;
      if (!currentPendingPOI) {
        ...
        throw new Error(
          "No pending POI position. Please try creating the POI again.",
        );
      }
          ...
          if (filesToUpload.length === 1) {
            ...
            if (!uploadResponse.ok) {
              ...
              try {
                ...
                throw new Error(
                  errorJson.error ||
                    `Failed to upload file ${file.name}: ${uploadResponse.status} ${uploadResponse.statusText}`,
                );
              } catch (parseError) {
                throw new Error(
                  `Failed to upload file ${file.name}: ${uploadResponse.status} ${uploadResponse.statusText}`,
            ...
            if (
              ...
              uploadedFiles = uploadResult.uploadedFiles.map(
      ...
      if (isEditing && selectedPOI?.originalFilenames) {
        ...
        mergedOriginalFilenames =
          data.originalFilenames || selectedPOI.originalFilenames;
      }
      ...
      const saveResponse = await fetch(
        isEditing ? "/api/poi/update" : "/api/poi/save",
        {
      ...
      if (!saveResponse.ok) {
        ...
        throw new Error(
          `Failed to ${isEditing ? "update" : "save"} POI data: ${saveResponse.status} ${saveResponse.statusText}`,
        );
      }
      ...
      if (isEditing) {
        setPois((prev) =>
          prev.map((poi) => (poi.id === poiId ? poiData : poi)),
        );
      } else {
    ...
    const poiHotspotsRef = useRef<
      { poi: POIData; element: HTMLElement; hotspot: any }[]
    >([]);
    ...
    const createPOIHotspots = useCallback(() => {
      ...
      try {
        ...
        const currentPOIs = pois.filter(
          (poi) => poi.panoramaId === currentPanoramaId,
        );
        ...
        currentPOIs.forEach((poi) => {
          ...
          `;
            ...
            console.error(
              `Failed to create POI hotspot for ${poi.name}:`,
              error,
            );
          }
        });
    ...
    return (
      <>
        ...
        <div ref={containerRef} className={styles.poiContainer} />
