...
let rceditPath = null;
const electronBuilderCache = path.join(process.env.LOCALAPPDATA || process.env.APPDATA, 'electron-builder', 'Cache', 'winCodeSign');

try {
    if (fs.existsSync(electronBuilderCache)) {
        const winCodeSignDirs = fs.readdirSync(electronBuilderCache)
            .filter(dir => fs.statSync(path.join(electronBuilderCache, dir)).isDirectory())
            .sort()
        ...
        for (const dir of winCodeSignDirs) {
            ...
            const rceditIa32 = path.join(electronBuilderCache, dir, 'rcedit-ia32.exe');
...
if (!rceditPath) {
    const fallbackPaths = [
        ...
        path.join(__dirname, '..', 'node_modules', 'electron-builder', 'node_modules', 'app-builder-bin', 'win', 'x64', 'rcedit.exe')
    ];
...
if (!rceditPath) {
    console.log('❌ rcedit executable not found in electron-builder cache or fallback locations');
...
try {
    ...
    if (sizeAfter > sizeBefore) {
        ...
        try {
            ...
            console.log('⚠️  Warning: Could not clear icon cache:', cacheError.message);
        }
        
    } else {
        console.log('⚠️  Warning: File size did not increase, icon may not have been embedded');
    }
