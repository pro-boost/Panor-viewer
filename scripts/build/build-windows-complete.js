...
try {
  ...
  if (fs.existsSync("dist")) {
    try {
      ...
      console.warn(
        "⚠️ Could not fully clean dist directory, but continuing..."
      );
  ...
  if (fs.existsSync(standaloneSource)) {
    execSync(
      `robocopy "${standaloneSource}" "${electronAppTarget}" /E /R:3 /W:1 /NP /NDL /NC /NS /NJH /NJS`,
      { stdio: "inherit" }
    );
  ...
  if (fs.existsSync(desktopSource)) {
    execSync(
      `robocopy "${desktopSource}" "${desktopTarget}" /E /R:3 /W:1 /NP /NDL /NC /NS /NJH /NJS`,
      { stdio: "inherit" }
    );
...
pause`;
  ...
  console.log(
    "🔧 To run the application, execute: dist\\electron-app\\start.bat"
  );
} catch (error) {
