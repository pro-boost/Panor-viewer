...
function generateConfig(csvFile, outputFile = "config.json", projectPath = "") {
  ...
  // Detect column format by checking the first row
  const firstRow = rows[0];
  const hasStandardFormat = "pano_pos_x" in firstRow;
  const hasSimpleFormat = "x" in firstRow;

  if (!hasStandardFormat && !hasSimpleFormat) {
    throw new Error(
      'CSV file does not have the required position columns. Expected either "pano_pos_x, pano_pos_y, pano_pos_z" or "x, y, z"',
    );
  }

  console.log(
    `Detected CSV format: ${hasStandardFormat ? "Standard (pano_pos_x, etc.)" : "Simple (x, y, z)"}`,
  );
  ...
  for (const row of rows) {
    ...
    if (hasStandardFormat) {
      // Standard format: pano_pos_x, pano_pos_y, pano_pos_z, pano_ori_w, pano_ori_x, pano_ori_y, pano_ori_z
      pos = [
        parseFloat(row.pano_pos_x),
        parseFloat(row.pano_pos_y),
        parseFloat(row.pano_pos_z),
      ];
      ori = [
        parseFloat(row.pano_ori_w),
        parseFloat(row.pano_ori_x),
        parseFloat(row.pano_ori_y),
        parseFloat(row.pano_ori_z),
      ];
    } else {
      // Simple format: x, y, z, w, x_rot, y_rot, z_rot
      pos = [parseFloat(row.x), parseFloat(row.y), parseFloat(row.z)];
      ori = [
        parseFloat(row.w),
        parseFloat(row.x_rot),
        parseFloat(row.y_rot),
        parseFloat(row.z_rot),
      ];
    }

    // Validate that we have valid numbers
    if (pos.some(isNaN) || ori.some(isNaN)) {
      console.warn(
        `Warning: Invalid numeric data in row for ${row.filename}, skipping...`,
      );
      continue;
    }
  ...
  if (panoramas.length === 0) {
    throw new Error("No valid panorama data found in CSV file");
  }

  console.log(`Processed ${panoramas.length} panoramas from CSV`);

  // Group Z positions into floor clusters
  const zValues = [...new Set(panoramas.map((p) => p.pos[2]))].sort(
    (a, b) => a - b,
  );
  const floors = [];
  ...
  for (const pano of panoramas) {
    ...
    if (currentFloor < 0) {
      // Basement floors might have lower ceilings
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--help":
        console.log(
          "Usage: node generate-marzipano-config.js --project <projectId> [--test]",
        );
        console.log("Options:");
        console.log(
