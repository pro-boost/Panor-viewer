...
function generateConfig(csvFile, outputFile = 'config.json', projectPath = '') {
    ...
    if (!hasStandardFormat && !hasSimpleFormat) {
        throw new Error('CSV file does not have the required position columns. Expected either "pano_pos_x, pano_pos_y, pano_pos_z" or "x, y, z"');
    }
    
    console.log(`Detected CSV format: ${hasStandardFormat ? 'Standard (pano_pos_x, etc.)' : 'Simple (x, y, z)'}`);
    ...
    for (const row of rows) {
        ...
        if (hasStandardFormat) {
            ...
            ori = [parseFloat(row.w), parseFloat(row.x_rot), parseFloat(row.y_rot), parseFloat(row.z_rot)];
        }
        
        // Validate that we have valid numbers
        if (pos.some(isNaN) || ori.some(isNaN)) {
            console.warn(`Warning: Invalid numeric data in row for ${row.filename}, skipping...`);
            continue;
        }
